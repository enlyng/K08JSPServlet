...
<body>
	...
	나머지는 0이므로 true가 출력된다.
	 -->
	<c:if test='${number mod 2 eq 0 }' var="result">${number }는 짝수입니다.<br>
	</c:if>
	result : ${result }<br>
	
	
	<!-- 
	JSTL의 if태그는 else구문이 따로 없으므로 두개의 if태그를 통해 else와
		같이 사용해야 한다.
	...
	<c:if test="${string eq 'Java' }" var="result2">문자열은 Java입니다.<br>
	</c:if>
	<!-- result2의 not을 붙여 반대의 조건을 만든다. 따라서 else와 같은
	구문이 된다. -->
	<c:if test="${not result2 }">'Java'가 아닙니다. <br>
	</c:if>
	
	<h4>조건식 주의사항</h4>
	<c:if test="100" var="result3">EL이 아닌 정수를 지정하면 false<br>
	</c:if>
	result3 : ${result3 }<br>
	<c:if test="tRuE" var="result4">대소문자 구분없이 "tRuE"인 경우 true<br>
	</c:if>
	result4 : ${result4 }<br>
	<c:if test=" ${true } " var="result5">EL 양쪽에 빈 공백이 있는 경우 false<br>
	</c:if>
	result5 : ${result5 }<br>
	...
	<c:if test="${not empty param.user }">
		<c:if test="${param.user eq 'kosmo' && param.pass eq '1234' }" var="result">${param.user }님 하이룽~</c:if>
		<c:if test="${not result }">아이디와 비번을 확인하세요</c:if>
	</c:if>
	<form method="get">
