...
//application 내장 객체를 DAO객체의 생성자로 전달하여 생성자에서 web.xml의 컨텍스트 초기화
...
//전체페이지 수를 계산한다.(전체게시물의개수/페이지당 게시물개수 = 결과의 올림처리)
...
List<BoardDTO> boardLists = dao.selectListPage(param); //게시물 목록닫기
...
<%
    ...
    int virtualNum = 0;
	int countNum = 0;
	//게시물의 개수만큼, 즉 List컬렉션에 저장된 DTO객체의 개수만큼 반복한다.
    for (BoardDTO dto : boardLists)
    {
    	//전체 레코드수를 1씩 차감하면서 리스트에 출력한다.
    	//현재 페이지 번호를 반복해서 출력
...
%>
    ...
    <table border="1" width="90%">
        ...
        request.getRequstURI() : request내장객체를 통해 현재 페이지에서
        HOST부분을 제외한 전체 경로명을 얻어올 수 있다. 여기서 얻은
        경로명을 통해 "경로명?pageNum=페이지번호"와 같은 바로가기 링크를
        만들게 된다.
         -->
        <td>
