...
public class BoardDAO extends JDBConnect {
	...
	/*
	 * board테이블에 저장된 게시물의 개수를 카운트하기 위한 메서드. 카운트 한 결과값을 통해 목록 출력시 게시물의 순번을 출력한다. 만약
	 * 검색어가 있는 경우를 대비해서 Map컬렉션을 매개변수로 선언한다.
	 */
...
//		List<BoardDTO> bbs = new Vector<BoardDTO>();
		...
		/*
		 * 목록에 출력할 게시물을 출력하기 위한 쿼리문으로 항상 일련번호(작성순)의 역순(내림차순)으로 정렬해야한다. 게시판의 목록은 최근 게시물이
		 * 제일 위쪽에 노출되기 때문이다.
		 */
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			//인파라미터 있는 동적 쿼리문 작성(사용자의 입력에 따라 달라짐)
	...
	public BoardDTO selectView(String num) {
		...
		try {
			...
			//일련번호는 중복되지 않으므로if문으로 처리한다.
			//게시판 목록처럼 여러개의 레코드를 가져온다면 while문을 사용하면 된다.
			if(rs.next()) {
				//DTO에 레코드의 내용을 추가한다. 인덱스를 써도되고 컬럼명을 써도 된다.
	...
	public int deletePost(BoardDTO dto) {
		...
		try {
			...
			/*
			insert의 경우 하나씩만 삽입 가능하기 때문에 0또는 1이 반환되나,
			delete의 경우 한꺼번에 삭제가 가능하기 때문에 100이라는 숫자값도
			반환될 수 있다.
			 */
		}
		catch(Exception e) {
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		//검색조건 추가. 검색어가 있는 경우에만 where절 추가
		if (map.get("searchWord") != null) {
			query += " WHERE " + map.get("searchField") + " LIKE '%" + map.get("searchWord") + "%' ";
		}
...
//		sb.append(" (select * from board order bu num desc) tb) ");
