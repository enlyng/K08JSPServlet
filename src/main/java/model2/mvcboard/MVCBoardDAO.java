...
//모델 2방식의 자료실형 게시판에서 사용할 DAO클래스 생성
...
public class MVCBoardDAO extends DBConnPool {
	...
	//mvcboard테이블의 게시물 개수를 카운트하여 반환한다.
	...
	/*
	모델 1방식에서는 board테이블 및 BoardDTO클래스를 사용했지만
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			/*
			select문에 count()함수를 사용하는 경우 조건(where절)에 만족하는
			레코드가 없을때는 0, 만족하는 레코드가 있을때는 1 이상의 정수값이 반환된다.
			즉 어떤 경우에도 결과값이 있으므로 next()호출시 if문이 필요하지 않다.
			 */
			rs.next();
			/*
			if(rs.next())
				=> 일반적인 select문을 사용하는 경우 1개의 레코드만 반환될때 사용한다.
				주로 일련번호를 조건으로 추가하는 경우인데, 만족하는 레코드가 없다면
				next()를 사용할때 에러가 발생한다.
